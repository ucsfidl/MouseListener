function mouseListener_rpi3(varargin)
...
% If runthresh argument is omitted, it defaults to 2 cm/sec
...
RPiport = 8931;
psychstimhost = 'mps-pc38.cin.ucsf.edu'; % runs PsychStimController
...
while 1
    ...
    if dT(samp) > (dT(samp0) + t_int)
        ...
        disp(sprintf('speed dx: %.2f dy: %.2f dy2: %.2f time: %.4f speed: %.2f',sqrt(dx),sqrt(dym1),sqrt(dym2),deltaT,speed));
        samp0 = samp;
        dym1 = 0.;
        dym2 = 0.;
        dx =0.;
